...
3. [Tạo Supabase project & lấy DATABASE_URL](#3-tạo-supabase-project--lấy-database_url)
...
└── External:
      ...
      Vercel Blob (free tier, cùng account)
```
...
- **Tài khoản [Supabase](https://supabase.com)** (miễn phí, Free tier)
...
## 3. Tạo Supabase project & lấy DATABASE_URL

### 3.1. Tạo project

1. Đăng nhập [supabase.com](https://supabase.com) → nhấn **New project**.
2. Điền tên project, mật khẩu database (lưu lại mật khẩu này), chọn region gần VN nhất (Singapore).
3. Chờ project khởi tạo (thường 1–2 phút).

### 3.2. Lấy Connection Pooler URL (quan trọng)

Dùng **Connection Pooler** (pgbouncer, port **6543**) thay vì direct connection. FastAPI serverless cần điều này để tránh cạn pool kết nối.

1. Trong dashboard Supabase → **Settings** (bánh răng, cột trái dưới) → **Database**.
2. Cuộn xuống phần **Connection pooling**.
3. Đảm bảo chọn mode **Transaction** (PgBouncer).
4. Copy chuỗi có dạng:
   ```
   postgresql://postgres.[project-ref]:[password]@aws-0-ap-southeast-1.pooler.supabase.com:6543/postgres
   ```
5. Đây chính là giá trị `DATABASE_URL` bạn sẽ dùng.

> **Không dùng direct URL (port 5432)** khi deploy lên Vercel serverless — mỗi invocation mở connection mới, dùng direct sẽ cạn pool nhanh.
...
|---|---|---|
| `DATABASE_URL` | Supabase pooler URL (bước 3.2) | **Bắt buộc** — dùng port 6543 |
...
```bash
cd apps/api && DATABASE_URL=$DATABASE_URL alembic upgrade head && cd ../.. && pnpm exec nx run web:build
```

> Cách này chạy migration tự động trước mỗi lần build. Phù hợp khi bạn có migration mới sau này.
> **Lưu ý:** build sẽ cần `DATABASE_URL` tồn tại lúc build — đảm bảo biến đã set và scope **Production**.
...
# Chạy migration nhắm thẳng Supabase
DATABASE_URL="postgresql://postgres.[ref]:[password]@...pooler.supabase.com:6543/postgres" \
  alembic upgrade head
```

> Thay `postgresql://...` bằng Supabase pooler URL của bạn (bước 3.2).
...
Sau khi migration chạy xong, tạo admin bằng script (chạy từ máy local, nhắm Supabase):
...
2. Nguyên nhân thường gặp: `DATABASE_URL` chưa set hoặc sai format.
3. Đảm bảo dùng **pooler URL (port 6543)**, không phải direct URL (port 5432).

### 502 Bad Gateway khi gọi API

Thường do connection DB thất bại:
- Kiểm tra `DATABASE_URL` đúng pooler Supabase (port 6543).
- Kiểm tra project Supabase còn active (free tier pause sau 1 tuần không dùng).
- Vào Supabase dashboard → **Resume** nếu project bị pause.
